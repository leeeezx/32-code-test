...
// 新增的long_delay_ms函数，用于处理超过65535ms的延时
void long_delay_ms(uint32_t nms)
{
    uint32_t repeat = nms / 65535;  // 计算需要多少次最大延时
    uint16_t remain = nms % 65535;  // 剩余的时间

    for (uint32_t i = 0; i < repeat; i++)
    {
        delay_ms(65535); // 调用最大延时
    }
    if (remain)
    {
        delay_ms(remain); // 调用剩余的时间
    }
}
