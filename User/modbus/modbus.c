...
/*********************设置SPD0和SPD1的值，选择p1.009寄存器的速度值0*********************/
	if(biaozhiwei[0])	
	{
		biaozhiwei_jiaobiao=0;  
		is_fasong_16=1;
		addr=0x040E;   //p4.007地址
		data=0x0004;   //设置SPD0为1，SPD1为0
	}	
/*********************设置SPD0和SPD1的值，选择p1.010寄存器的速度值50r/min*********************/
	if(biaozhiwei[1])	
	{
		...
		is_fasong_16=1;
		addr=0x040E;   //p4.007地址
		data=0x0008;   //设置SPD0为0，SPD1为1
	}	
/***********设置SPD0和SPD1的值，选择p1.011寄存器的速度值-50r/min**********/
	if(biaozhiwei[2])	
	{
		biaozhiwei_jiaobiao=1;  
		is_fasong_16=1;
		addr=0x040E;   //p4.007地址
		data=0x000C;   //设置SPD0为1，SPD1为1
}
/***********设置p3.006值，使DI1和DI2为系统参数P4.007控制**********/
	if(biaozhiwei[3])	
	{
		biaozhiwei_jiaobiao=1;  
		is_fasong_16=1;
		addr=0x030C;   //p3.006地址
		data=0x000C;   //设置DI3和DI4为通讯控制
}
	}
...
void fasong03(void)
{
	Modbus_TX_FrameFlag=0;
	Modbus_TX_BUFF[0]=qingqiudizhi;
	Modbus_TX_BUFF[1]=0x03;
	Modbus_TX_BUFF[2]=jicunqidizhi/256;
	Modbus_TX_BUFF[3]=jicunqidizhi%256;
	Modbus_TX_BUFF[4]=jicunqichangdu/256;
	Modbus_TX_BUFF[5]=jicunqichangdu%256;
...
void fasong06(uint16_t jicunqi,uint16_t neirong)
{
	Modbus_TX_FrameFlag=0;
	Modbus_TX_BUFF[0]=qingqiudizhi;
	Modbus_TX_BUFF[1]=0x06;
	Modbus_TX_BUFF[2]=jicunqi/256;
	Modbus_TX_BUFF[3]=jicunqi%256;
	Modbus_TX_BUFF[4]=neirong/256;
	Modbus_TX_BUFF[5]=neirong%256;
	calCRC = GetCRC16(Modbus_TX_BUFF,6);  //CRC检查
	Modbus_TX_BUFF[6]=calCRC/256;
	Modbus_TX_BUFF[7]=calCRC%256;
	Modbus_SendData(Modbus_TX_BUFF,8);    //全部发送
}

void MODBUS_MASTER_SEND_16( uint16_t Reg_Start_Addr,uint16_t Reg_Len,uint16_t data)
{

	
	Modbus_TX_FrameFlag=0;
	Modbus_TX_BUFF[0]=1;
	Modbus_TX_BUFF[1]=0x10;
	Modbus_TX_BUFF[2]=Reg_Start_Addr/256;
	Modbus_TX_BUFF[3]=Reg_Start_Addr%256;
	Modbus_TX_BUFF[4]=Reg_Len/256;
	Modbus_TX_BUFF[5]=Reg_Len%256;
	Modbus_TX_BUFF[6]=Reg_Len*2;
	Modbus_TX_BUFF[7]=data/256;
	Modbus_TX_BUFF[8]=data%256;
	Modbus_TX_BUFF[9]=0;
	Modbus_TX_BUFF[10]=0;


	calCRC = GetCRC16(Modbus_TX_BUFF,11);
	Modbus_TX_BUFF[11]=calCRC/256;
	Modbus_TX_BUFF[12]=calCRC%256;
...
{
//if(Modbus_TX_FrameFlag==1)
	{
		shunxuzhixing();
		if(is_fasong_06)
		{
			fasong06(jicunqi_06,neirong_06);
			is_fasong_06=0;
		}
		else if(is_fasong_16) 
		{
			MODBUS_MASTER_SEND_16(addr,2,data);
			is_fasong_16=0;
		}
//		else
//		{
